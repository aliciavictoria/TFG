...
LOG_FULL_RESPONSES = False   # ya no controla si se guarda crudo (eso ahora es SIEMPRE);
                              # solo controla si el crudo también se copia dentro del JSON de métricas.

# ─── Captura cruda SIEMPRE, desacoplada de la extracción ──────────────────────
# Cada llamada al modelo se guarda ANTES de intentar parsear nada. Si el
# extractor falla o tiene un bug, el texto original sigue disponible en
# raw_logs/ y se puede reprocesar sin volver a llamar a la API
# (ver rescore_from_raw.py).

RAW_LOG_DIR = "raw_logs"

def _log_raw(puzzle_id: str, model_key: str, strategy: str, step_label,
             prompt: str, response: str) -> None:
    Path(RAW_LOG_DIR).mkdir(exist_ok=True)
    fname = f"{RAW_LOG_DIR}/{puzzle_id}_{model_key}.jsonl"
    record = {
...
def _has_repetition(text: str, threshold: int = 6) -> bool:
    """Detecta si la respuesta tiene un bloque de líneas idénticas repetidas
    (síntoma del bucle degenerativo del modelo en la sección DICTIONARY)."""
    lines = [l.strip() for l in text.split('\n') if l.strip()]
    if len(lines) < threshold:
        return False
    for i in range(len(lines) - threshold + 1):
        if len(set(lines[i:i + threshold])) == 1:
            return True
    return False
...
def call_model(prompt: str, model_key: str, api_keys: dict,
    ...
    max_out = 8000 if "gpt-oss" in model_key else 4096

    def _make_payload(temperature: float) -> dict:
        return {
    ...
    for attempt in range(4):
        # Intentos 0-1: temperatura 0.0 (determinista)
        # Intentos 2-3: temperatura 0.4 (rompe bucles profundos — el 8B
        #               necesita más diversidad que el 120B para salir del loop)
        temperature = 0.0 if attempt < 2 else 0.4
        if attempt == 2:
            print("    [retry] Bucle de repetición detectado — reintentando con temperature=0.4")
        payload = _make_payload(temperature)
        ...
        # Si hay bucle de repetición, reintentar con temperature mayor
        if _has_repetition(text) and attempt < 3:
            continue

        return text
    raise RuntimeError("No se pudo obtener respuesta válida tras 4 intentos")
...
def _clean(text: str) -> str:
    ...
    text = re.sub(r'\s*===\s*END\s*===.*$', '', text, flags=re.IGNORECASE).strip()
...
_HEADER_RE = re.compile(r'\bTRANSLATIONS\s*:', re.IGNORECASE)
_NUM_LINE_RE = re.compile(r'^\s*\**\(?\s*(\d{1,2})\s*[\.\)\:]\)?\**\s*(.*)$')
_STOP_SECTION_RE = re.compile(
    r'^\s*\**\s*(?:===\s*END|ANALYSIS|DICTIONARY|GRAMMAR\s*RULES?|NOTE|EXPLANATION|CRITIQUE)\s*[:=\*]?',
    re.IGNORECASE,
)
...
def extract_translations(response: str, num_expected: int) -> list:
    """
    Extrae traducciones de la respuesta libre del modelo, INDEXANDO por el
    número que el propio modelo escribe ("3. ..." -> siempre va a la
    posición 3), no por orden de aparición. Esto es inmune al bug clásico
    de desplazamiento: si el modelo se salta o el parser no reconoce una
    línea, las demás NO se corren de sitio ni se emparejan con la
    referencia equivocada.

    Devuelve una lista de longitud num_expected. Las posiciones no
    encontradas quedan como None (en vez de desaparecer silenciosamente).
    compute_metrics() se encarga de evaluar solo las posiciones no-None,
    dejando constancia explícita de cuáles faltaron.
    """
    header_matches = list(_HEADER_RE.finditer(response))
    h = header_matches[-1] if header_matches else None  # última aparición = la sección final real
    section = response[h.end():] if h else response

    out = [None] * num_expected
    current_idx, current_lines = None, []

    def _flush():
        nonlocal current_idx, current_lines
        if current_idx is not None and 1 <= current_idx <= num_expected:
            text = _clean(' '.join(current_lines).strip())
            if (text and 1 <= len(text) and
                    not _is_instruction(text) and
                    not _is_meta_analysis(text) and
                    not _DICT_ENTRY_RE.match(text) and
                    not _SKIP_RE.match(text)):
                out[current_idx - 1] = text
        current_idx, current_lines = None, []
    ...
    for line in section.split('\n'):
        ...
        if m:
            ...
            stripped = line.strip()
            if not stripped:
                _flush()  # línea en blanco = fin de esta traducción (no engullir párrafos sueltos)
            else:
                current_lines.append(stripped)  # traducción envuelta en varias líneas
    _flush()

    # Red de seguridad: si NO se reconoció ningún número (el modelo ignoró
    # el formato pedido y escribió las traducciones en líneas sueltas sin
    # numerar), se intenta un último recurso secuencial. Se marca aparte
    # para que quede claro que la correspondencia posicional aquí NO está
    # garantizada por un índice explícito del modelo.
    if all(x is None for x in out):
        ...
        for line in section.split('\n'):
            if _STOP_SECTION_RE.match(line):
                break
            s = _clean(line.strip())
            if (s and 1 < len(s) < 300 and
                    not _is_instruction(s) and not _is_meta_analysis(s) and
                    not _DICT_ENTRY_RE.match(s) and not _SKIP_RE.match(s)):
                candidate_lines.append(s)
        for i, s in enumerate(candidate_lines[:num_expected]):
            out[i] = s
...
def normalize_reference(ref: str) -> str:
    """Elimina marcadores de número gramatical (sg)/(pl) de las referencias.
    Estos marcadores son anotaciones del puzzle de Linguini, no inglés natural.
    El modelo produce 'you' en lugar de 'you(sg)', lo que es correcto en inglés.
    Confirmado por el tutor: no deben penalizarse en las métricas.
    Ejemplos: 'Do you(sg) sleep?' → 'Do you sleep?'
              'Do they know you(pl)?' → 'Do they know you?'
    """
    return re.sub(r'\s*\([sp][gl]\.?\)', '', ref).strip()
...
def compute_metrics(hypotheses: list, references: list) -> dict:
    """
    hypotheses puede venir con huecos (None) en posiciones no extraídas.
    El emparejamiento es SIEMPRE por índice (hypotheses[i] <-> references[i]).

    Decisión documentada en la memoria (Tabla 5.4, "Evaluación parcial de
    respuestas incompletas"): las posiciones ausentes se completan con
    cadena vacía "" antes de calcular las métricas de corpus, para que
    penalicen correctamente en vez de excluirse silenciosamente (lo que
    podía dar una puntuación artificialmente alta sobre un subconjunto).
    missing_indices / complete se mantienen como metadatos de diagnóstico.

    Las referencias se normalizan eliminando los marcadores (sg)/(pl) antes
    del cálculo de métricas (véase normalize_reference).
    """
    try:
    ...
    if not hypotheses or not references:
        return {
            ...
            "num_expected": n_expected, "missing_indices": list(range(n_expected)),
            "complete": n_expected == 0,
        }

    # Normalización: eliminar (sg)/(pl) de AMBOS lados antes de comparar
    references_norm = [normalize_reference(r[0] if isinstance(r, list) else str(r))
                       for r in references]
    n = min(len(hypotheses), len(references_norm))

    missing_indices = [i for i in range(n) if hypotheses[i] is None]
    # Relleno con cadena vacía (Tabla 5.4), NUNCA se compacta/desplaza
    hyps_raw = [hypotheses[i] if hypotheses[i] is not None else "" for i in range(n)]
    hyps = [normalize_reference(h) for h in hyps_raw]   # normalizar hipótesis también
    refs = references_norm[:n]
    ...
    return {
        ...
        "hypotheses":         hyps,
        "references":         refs,
        "num_evaluated":      n - len(missing_indices),
        "num_expected":       n_expected,
...
def _translations_format_block(n: int, label: str = "translation") -> str:
    """
    Bloque de formato para la sección TRANSLATIONS, generado con el número
    REAL de frases esperadas (no un ejemplo genérico de 2 líneas), con
    instrucción explícita de no saltarse ningún número y un marcador de
    cierre. Reduce extracciones incompletas o con huecos sin marcar.
    """
    lines = "\n".join(f"{i}. [{label} only]" for i in range(1, n + 1))
...
{_translations_format_block(len(test_items))}"""
...
{_translations_format_block(len(test_items))}"""
        ...
        t0       = time.time()
        response = call_model(prompt, model_key, api_keys)
        _log_raw(puzzle["id"], model_key, "incremental_combinations", idx + 1, prompt, response)
        latency  = round(time.time() - t0, 2)
...
{_translations_format_block(len(test_items))}"""
        ...
        t0       = time.time()
        response = call_model(prompt, model_key, api_keys)
        _log_raw(puzzle["id"], model_key, "step_by_step", stage, prompt, response)
        latency  = round(time.time() - t0, 2)
...
{_translations_format_block(len(test_items))}"""
    ...
    t0       = time.time()
    response = call_model(prompt, model_key, api_keys)
    _log_raw(puzzle["id"], model_key, "human_inspiration", 1, prompt, response)
    latency  = round(time.time() - t0, 2)
...
{_translations_format_block(len(test_items))}"""
    ...
    _log_raw(puzzle["id"], model_key, "self_correction", "initial_draft", prompt_t1, response1)
    ...
    translations1_block = "\n".join(
        f"  {i+1}. {h if h is not None else '[no answer given]'}"
        for i, h in enumerate(hyps1)
    ) if any(h is not None for h in hyps1) else "  (no translations extracted)"
...
CRITICAL: Your task is to {trans_instr}. Every line in TRANSLATIONS must be
in {'English' if direction == 'lang2en' else puzzle['language']}.
...
{_translations_format_block(len(test_items), label="final translation")}"""
    print("  [M5] Turno 2: autocorrección")
    t0        = time.time()
    response2 = call_model(prompt_t2, model_key, api_keys)
    _log_raw(puzzle["id"], model_key, "self_correction", "self_correction", prompt_t2, response2)
...
if __name__ == "__main__":
    ...
    # ── Llama-3.1-8B — 11 puzzles →EN, todas las metodologías ───────────────
    PUZZLES = [
        "puzzles/linguini_012018020100.json",  # Hakhun →EN        [Sino-Tibetana]
        "puzzles/linguini_012022030100.json",  # Ngemba →EN        [Niger-Congo]
        "puzzles/linguini_012019010100.json",  # Yonggom →EN       [Trans-New Guinea]
        "puzzles/linguini_012015040100.json",  # Warlpiri →EN      [Pama-Nyungan]
        "puzzles/linguini_012023020100.json",  # Apurinã →EN       [Arawakan]
        "puzzles/linguini_012023030100.json",  # Coastal Marind →EN [Anim]
        ...
        "puzzles/linguini_012008050100.json",  # Inuktitut →EN     [Eskaleut]
        "puzzles/linguini_012005010100.json",  # Tzeltal →EN       [Mayan]
